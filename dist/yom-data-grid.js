(function webpackUniversalModuleDefinition(root, factory) {
	...
	else root["YomDataGrid"] = factory(root["$"]);
		...
		/******/ [
			...
			/***/ function(module, exports) {
				...
				function render($data, $opt) {
					...
					var i18n = $data.i18n,
						...
						dateWithoutYear = $data.dateWithoutYear || "yyyy-mm",
						dateFormat = $data.dateFormat || "yyyy-mm-dd",
						datetimeFormat = $data.datetimeFormat || "yyyy-mm-dd hh:ii",
						datetimeWithSeconds =
							$data.datetimeWithSeconds || "yyyy-mm-dd hh:ii:ss",
					...
					_$out_ +=
						'<h3><i class="fa fa-filter"></i> ' +
						$encodeHtml(column.name || i18n.filter) +
						"</h3>";
					...
					_$out_ +=
						'<div data-column-id="' +
						$encodeHtml(column.id) +
						'"><form><div class="alert alert-danger hidden"></div><div class="filter-option ' +
						(filterCriteria.findEmpty ? "hidden" : "") +
						'">';
					if (type == "set") {
						...
						if (filterOption.autoComplete) {
							_$out_ +=
								'<div class="form-group"><input type="text" value="" class="form-control auto-complete-box" /></div>';
						} else {
							_$out_ +=
								'<a class="btn-check-all" href="javascript:;">' +
								i18n.checkAll +
								'</a> / <a class="btn-uncheck-all" href="javascript:;">' +
								i18n.uncheckAll +
								'</a><div class="set-container">';
						...
						var compareType =
							filterCriteria.compareType || filterOption.defaultCompareType;
						_$out_ +=
							'<div class="form-group"><select name="compareType" class="form-control"><option value="eq" ' +
							(compareType == "eq" ? "selected" : "") +
							">" +
							i18n.eq +
							'</option><option value="lt" ' +
							(compareType == "lt" ? "selected" : "") +
							">" +
							i18n.lt +
							'</option><option value="gt" ' +
							(compareType == "gt" ? "selected" : "") +
							">" +
							i18n.gt +
							'</option><option value="range" ' +
							(compareType == "range" ? "selected" : "") +
							">" +
							i18n.range +
							'</option></select></div><div class="form-group ' +
							(compareType == "range" ? "" : "hidden") +
							'"><input name="fromValue" type="number" maxlength="10" step="' +
							(filterOption.step || "0.01") +
							'" value="' +
							$encodeHtml(
								filterCriteria.fromValue || filterCriteria.fromValue === 0
									? filterCriteria.fromValue
									: ""
							) +
							'" placeholder="' +
							i18n.gte +
							...
							(compareType == "range" ? "" : "hidden") +
							'"><input name="toValue" type="number" maxlength="10" step="' +
							(filterOption.step || "0.01") +
							'" value="' +
							$encodeHtml(
								filterCriteria.toValue || filterCriteria.toValue === 0
									? filterCriteria.toValue
									: ""
							) +
							'" placeholder="' +
							i18n.lte +
							'" class="form-control" /></div><div class="form-group ' +
							(compareType != "range" ? "" : "hidden") +
							'"><input name="value" type="number" maxlength="10" step="' +
							(filterOption.step || "0.01") +
							'" value="' +
							$encodeHtml(
								filterCriteria.value || filterCriteria.value === 0
						...
						_$out_ +=
							'<div class="form-group"><div class="datetimepicker-component input-group date date-from" data-date="' +
							$encodeHtml(filterCriteria.fromDisplay || "") +
							'" data-date-format="' +
							$encodeHtml(
								type == "datetime"
									? filterOption.showSeconds
										? datetimeWithSeconds
										: datetimeFormat
									: type == "month"
									? dateWithoutYear
							...
							'" data-value="' +
							$encodeHtml(filterCriteria.fromValue || "") +
							'"><input class="form-control" type="text" name="fromDate" value="' +
							$encodeHtml(filterCriteria.fromDisplay || "") +
							'" placeholder="' +
							i18n.start +
							'" readonly /><div class="input-group-addon"><i class="fa fa-calendar" /></div></div></div><div class="form-group"><div class="datetimepicker-component input-group date date-to" data-date="' +
							$encodeHtml(filterCriteria.toDisplay || "") +
							'" data-date-format="' +
							$encodeHtml(
								type == "datetime"
									? filterOption.showSeconds
							...
							'" data-value="' +
							$encodeHtml(filterCriteria.toValue || "") +
							'"><input class="form-control" type="text" name="toDate" value="' +
							$encodeHtml(filterCriteria.toDisplay || "") +
							'" placeholder="' +
							i18n.end +
							'" readonly /><div class="input-group-addon"><i class="fa fa-calendar" /></div></div></div>';
					} else {
						_$out_ +=
							'<div class="form-group"><input name="value" type="' +
							$encodeHtml(filterOption.onlyNumber ? "number" : "text") +
							'" value="' +
							$encodeHtml(filterCriteria.value || "") +
							'" class="form-control" maxlength="' +
							$encodeHtml(filterOption.maxLength || "") +
							'" /></div>';
					}
					_$out_ += "</div>";
					if (filterOption.enableEmpty) {
						_$out_ +=
							'<div class="checkbox"><label><input name="findEmpty" type="checkbox" ' +
							(filterCriteria.findEmpty ? "checked" : "") +
							" /> " +
							i18n.empty +
							"</label></div>";
					}
					_$out_ +=
						'<div class="row"><div class="col-xs-8"><button type="submit" class="btn btn-primary btn-sm btn-confirm">' +
						i18n.ok +
						'</button><button type="button" class="btn btn-default btn-sm" data-toggle="yom-data-grid-filter-panel">' +
						i18n.cancel +
						'</button></div><div class="col-xs-4 text-right">';
					if (filterMap[column.id]) {
						_$out_ +=
							'<a class="btn btn-link btn-remove" href="javascript:void(0);">' +
							i18n.clear +
							"</a>";
					}
			...
			/***/ function(module, exports) {
				...
				function render($data, $opt) {
					...
					_$out_ +=
						...
						i18n.setting +
						'</h3><div class="alert alert-danger hidden"></div><h4>' +
						i18n.displayAndSorting +
						'</h4><div class="yom-data-grid-setting-column-all"><label><input type="checkbox" /> ' +
						i18n.displayAll +
						'</label></div><div class="columns-container"><div class="yom-data-grid-setting-columns-container-inner">';
					for (var i = 0, l = columns.length; i < l; i++) {
						...
						if (column.searchOnly) {
							...
							_$out_ +=
								...
								$encodeHtml(column.id) +
								'" ' +
								(hiddenColumns.indexOf(column.id) >= 0 ? "" : "checked") +
								" /> " +
								$encodeHtml(column.name) +
								"";
						}
					...
					_$out_ +=
						'</div><button class="btn btn-default btn-sm yom-data-grid-setting-btn-move-up disabled" disabled><i class="fa fa-long-arrow-up "></i></button><button class="btn btn-default btn-sm yom-data-grid-setting-btn-move-down disabled" disabled><i class="fa fa-long-arrow-down"></i></button></div><h4>' +
						i18n.lockingColumns +
						'</h4><div class="lock-options">';
					for (var i = 1; i <= MAX_LOCKED_COLUMNS; i++) {
						_$out_ +=
							'<label class="radio-inline"><input type="radio" name="lock" value="' +
							i +
							'" ' +
							(lockColumnAmount == i ? "checked" : "") +
							" /> " +
							i +
							"</label>";
					}
					_$out_ +=
						'<label class="radio-inline"><input type="radio" name="lock" value="0" ' +
						(lockColumnAmount == 0 ? "checked" : "") +
						" /> " +
						i18n.noLocking +
						'</label></div><button type="button" class="btn btn-primary btn-sm yom-data-grid-btn-confirm-setting">' +
						i18n.ok +
						'</button><button type="button" class="btn btn-default btn-sm" data-toggle="yom-data-grid-setting-panel">' +
						i18n.cancel +
						"</button>";
					return _$out_;
				}
			...
			/***/ function(module, exports, __webpack_require__) {
				/* WEBPACK VAR INJECTION */ (function(global, module) {
					(function(global) {
						global.yomCssModuleHelper =
							global.yomCssModuleHelper ||
							function(className, cssContent, moduleUri) {
								var head =
								...
								window._yom_style_module_injected =
									window._yom_style_module_injected || {};
								if (!moduleUri) {
				...
				function render($data, $opt) {
					...
					for (i = 0, l = lockedColumns.length; i < l; i++) {
						...
						(function() {
							...
							columnWidth +=
								...
								(column.locked ? column.width : noScrollX ? 0 : column.width) +
								'px;"></colgroup>';
							columnHeader +=
								'<th class="' +
								(column.type == "checkbox"
									? "yom-data-grid-checkbox-cell"
									: "") +
								" " +
								(i == l - 1 ? "yom-data-grid-last-cell" : "") +
								" yom-data-grid-column-" +
								column.id.replace(/\./g, "-") +
								'"><div data-column-id="' +
								column.id +
								'" class="yom-data-grid-cell-inner yom-data-grid-header-cell-inner ' +
								("yom-data-grid-header-cell-inner-align-" +
							...
							if (column.type == "checkbox") {
								...
								if (!(maxSelection > 0)) {
									columnHeader +=
										'<input class="yom-data-grid-check-box-all" type="checkbox" ' +
								...
								if (filterMap[column.id]) {
									columnHeader +=
										'<a class="yom-data-grid-filter-remove-icon" href="javascript:void(0);" title="' +
										i18n.clearFilterCriteria +
										'"><i class="fa fa-filter icon-filter"></i><i class="fa fa-remove icon-remove"></i></a>';
								}
								if (column.headerRenderer) {
									columnHeader +=
										"" +
										column.headerRenderer.call(
											_this,
											column.name,
											i,
											column,
									...
									columnHeader +=
										'<a class="yom-data-grid-sortable" href="javascript:void(0);" onclick="return false" title="' +
										$encodeHtml(column.name) +
										'">' +
										(sortBy == sortKey
											? sortOrder == "desc"
												? '<span class="yom-data-grid-sort-arrow yom-data-grid-sort-arrow-down"></span>'
												: '<span class="yom-data-grid-sort-arrow yom-data-grid-sort-arrow-up"></span>'
											: '<span class="yom-data-grid-sort-arrow"></span>') +
										"" +
										column.name +
										"</a>";
								} else {
								...
								if (filterable) {
									columnHeader +=
										'<a href="javascript:void(0);" class="yom-data-grid-filter-icon"><i class="fa fa-filter"></i></a>';
								}
							}
							columnHeader += "</div></th>";
						})();
					...
					if (!noScrollX && width > 0) {
						if (noWidthScrollColumns.length) {
							if (
								width - lockedTableWidth - scrollTableWidth <
								noWidthScrollColumns.length * DEFAULT_COLUMN_WIDTH
							) {
					...
					if (!disableSetting) {
						_$out_ +=
							'<a class="yom-data-grid-setting-icon" href="javascript:void(0);"><i class="fa fa-cog"></i></a>';
					}
					_$out_ +=
						'<div class="yom-data-grid-setting-panel"></div><div class="yom-data-grid ' +
						(lockedDisplayColumns.length ? "yom-data-grid-locked" : "") +
						" " +
						(bordered ? "yom-data-grid-bordered" : "") +
						" " +
						(bordered == "h" || bordered == "v"
							? "yom-data-grid-bordered-" + bordered
							: "") +
						" " +
						(striped ? "yom-data-grid-striped" : "") +
						" " +
						(headerData.length > 0
							? "yom-data-grid-header-rows-" + headerData.length
							: "") +
						'" style="overflow: hidden;"><table border="0" cellspacing="0" cellpadding="0" style="width: 100%; height: 100%;"><tr>';
					if (lockedDisplayColumns.length) {
						_$out_ +=
							'<td class="yom-data-grid-columns-container" style="width: ' +
							lockedTableWidth +
							'px;"><div class="yom-data-grid-locked-columns" style="width: ' +
							lockedTableWidth +
							'px; overflow: hidden;"><div class="yom-data-grid-header"><table class="yom-data-grid-table" border="0" cellspacing="0" cellpadding="0" style="width: ' +
							lockedTableWidth +
							'px;">' +
							lockedColumnWidth +
							"<tbody><tr>" +
							...
							(function() {
								...
								if (renderData.length) {
									for (i = 0, l = renderData.length; i < l; i++) {
										...
										isPrevOdd = isOdd;
										rowClickable = opt.getRowClickable
											? opt.getRowClickable(i, item, isHeaderData)
											: !isHeaderData;
										_this._tmp.reactElements[i] =
											_this._tmp.reactElements[i] || [];
										_this._tmp.headerReactElements[i] =
											_this._tmp.headerReactElements[i] || [];
										_$out_ +=
											"<tr data-grid-" +
											(isHeaderData ? "header-row" : "row") +
											'="' +
											i +
											'" class="' +
											(i == l - 1 ? "yom-data-grid-last-row" : "") +
											" " +
											(isOdd ? "yom-data-grid-row-odd" : "") +
											" " +
											(rowClickable && opt.onRowClick
												? "yom-data-grid-row-clickable"
												: "") +
											" " +
											(opt.getRowClassName
												? opt.getRowClassName(i, item, isHeaderData)
												: "") +
											" " +
											(selectedIndex.length && selectedIndex.indexOf(i) >= 0
												? "yom-data-grid-row-checked"
												: "") +
											'">';
										for (j = 0, l2 = columns.length; j < l2; j++) {
											...
											cellClickable = opt.getCellClickable
												? opt.getCellClickable(
														i,
														item,
														column.id,
														column,
											...
											if (column.reactRenderer) {
												reactElement = column.reactRenderer.call(
													_this,
													columnValue || "",
													i,
													item,
												...
												if (reactElement) {
													_this._tmp[
														isHeaderData
															? "headerReactElements"
															: "reactElements"
													][i][columnIndex] = reactElement;
												} else {
													displayValue = $encodeHtml(columnValue || "");
												}
											} else if (column.renderer) {
												displayValue = column.renderer.call(
													_this,
													$encodeHtml(columnValue || ""),
													i,
													item,
													{
														columnIndex: columnIndex,
												...
												if (displayValue == null) {
													...
													displayValue =
														'<a class="yom-data-grid-cell-clickable" href="javascript:void(0);">' +
														displayValue +
														"</a>";
												}
											} else {
											...
											if (column.titleRenderer) {
												title = column.titleRenderer.call(
													_this,
													columnValue || "",
													i,
													item,
											...
											_$out_ +=
												'<td data-grid-column-id="' +
												column.id +
												'" id="yom-data-grid-' +
												name +
												"-" +
												(isHeaderData ? "header-cell" : "cell") +
												"-" +
												i +
												"-" +
												columnIndex +
												...
												(column.type == "sequence"
													? "yom-data-grid-sequence-cell"
													: column.type == "checkbox"
													? "yom-data-grid-checkbox-cell"
													: column.type == "form"
													? "yom-data-grid-form-cell"
													: "yom-data-grid-content-cell") +
												" " +
												(j == l2 - 1 ? "yom-data-grid-last-cell" : "") +
												" yom-data-grid-column-" +
												column.id.replace(/\./g, "-") +
												'"' +
												(reactElement
													? ' data-react-element="' +
													  ...
													  '"'
													: "") +
												'><div class="yom-data-grid-cell-inner yom-data-grid-cell-inner-' +
												(column.textAlign || "left") +
												'" title="' +
												$encodeHtml(title) +
												'">';
											if (column.type == "sequence") {
												...
												checkable =
													!checkbox ||
													!checkbox.checkable ||
													checkbox.checkable(item, i);
												if (isHeaderData) {
													_$out_ += "&nbsp;";
												} else if (checkable) {
													_$out_ +=
														'<input class="yom-data-grid-check-box" data-row-index="' +
														i +
														'" type="checkbox" ' +
														(selectedIndex.length &&
														selectedIndex.indexOf(i) >= 0
															? "checked"
															: "") +
														" />";
												} else {
									...
									_$out_ +=
										"\x3c!-- use a dummy row to enable horizental scroll --\x3e<tr>";
									for (j = 0, l2 = columns.length; j < l2; j++) {
										_$out_ +=
											'<td><div class="yom-data-grid-cell-inner-dummy"></div></td>';
									}
									_$out_ += "</tr>";
								}
							})();
						...
						_$out_ +=
							'</tbody></table></div><div class="yom-data-grid-body" style="' +
							(scrollX ? "overflow-x: scroll;" : "") +
							" width: " +
							lockedTableWidth +
							'px;"><table class="yom-data-grid-table" border="0" cellspacing="0" cellpadding="0" style="width: ' +
							lockedTableWidth +
							'px;">' +
							lockedColumnWidth +
							"<tbody>";
								...
								for (i = 0, l = renderData.length; i < l; i++) {
									...
									for (j = 0, l2 = columns.length; j < l2; j++) {
										...
										if (column.reactRenderer) {
											...
											if (reactElement) {
												_this._tmp[
													isHeaderData ? "headerReactElements" : "reactElements"
												][i][columnIndex] = reactElement;
											} else {
												displayValue = $encodeHtml(columnValue || "");
											}
					...
					if (scrollDisplayColumns.length) {
						_$out_ +=
							'<td class="yom-data-grid-columns-container"><div class="yom-data-grid-columns" style="left: ' +
							lockedTableWidth +
							'px;"><div class="yom-data-grid-header" style="width: 100%;"><table class="yom-data-grid-table" border="0" cellspacing="0" cellpadding="0" style="width: ' +
							(width > lockedTableWidth
								? width - lockedTableWidth + "px"
								: "100%") +
							';">' +
							scrollColumnWidth +
							"<tbody><tr>" +
						...
						_$out_ +=
							'</tbody></table></div><div class="yom-data-grid-body" style="' +
							(scrollX ? "overflow-x: scroll;" : "") +
							' width: 100%;"><table class="yom-data-grid-table" border="0" cellspacing="0" cellpadding="0" style="width: ' +
							(width > lockedTableWidth
								? width - lockedTableWidth + "px"
								: "100%") +
							';">' +
					...
					for (i = -1; i < data.length; i++) {
						...
						columns.forEach(function(column, j) {
							if (i === -1) {
								...
								rowData.push(
									encodeCsvCellValue(getCellValue(data[i], column, i, j))
								);
							}
						});
			...
			/***/ function(module, exports) {
				module.exports = {
					en: {
						...
						atLeastOneRangeValueRequired:
							"Range start and range end must be filled at least one",
			...
			/***/ function(module, exports, __webpack_require__) {
				...
				var YomDataGrid = function(holder, columns, opt) {
					...
					this._holder = $(holder);
					this._container = $(
						'<div class="yom-data-grid-container' +
							(opt.height == "100%" ? " yom-data-grid-container-height" : "") +
							(opt.sequence ? " yom-data-grid-container-sequence" : "") +
							'"></div>'
					).appendTo(holder);
					this._filterPanel = $(
						'<div class="yom-data-grid-filter-panel"></div>'
				...
				$.extend(YomDataGrid.prototype, {
					...
					_onScroll: function(evt) {
						...
						if (this._scrollHeader) {
							this._scrollLeft = this._scrollHeader.scrollLeft =
								target.scrollLeft;
						}
					...
					_onScrollLocked: function(evt) {
						...
						if (
							$.contains(scrollBody, evt.target) &&
							((scrollBody.scrollLeft > 0 &&
								scrollBody.scrollLeft + $(scrollBody).width() <
									scrollBody.scrollWidth) ||
								(scrollBody.scrollLeft === 0 && x > 0) ||
								(scrollBody.scrollLeft + $(scrollBody).width() ==
									scrollBody.scrollWidth &&
									x < 0))
						) {
						...
						if (Math.abs(x) > Math.abs(y)) {
							...
							if (
								(y > 0 &&
									scrollBody.clientHeight + top < scrollBody.scrollHeight) ||
								(y < 0 && top > 0)
							) {
								evt.preventDefault();
							}
					...
					_updateSettingColumnCheckboxAll: function() {
						...
						$(".yom-data-grid-setting-column-item input", this._container).each(
							function(i, item) {
								if (item.checked) {
									checkedCount++;
					...
					_hideSettingPanel: function(evt) {
						if (evt) {
							var target = $(evt.target);
							if (
								target.hasClass("yom-data-grid-setting-icon") ||
								target.closest(".yom-data-grid-setting-icon").length
							) {
								return;
							}
							if (
								(target.hasClass("yom-data-grid-setting-panel") ||
									target.closest(".yom-data-grid-setting-panel").length) &&
								target.attr("data-toggle") != "yom-data-grid-setting-panel"
							) {
								return;
							}
						}
					...
					_showSettingErrMsg: function(msg) {
						$(".alert-danger", this._settingPanel)
							.html(msg)
							.removeClass("hidden");
					},
					...
					_submitSettingForm: function() {
						var hiddenColumns = $(
							".columns-container input:not(:checked,:disabled)",
							this._settingPanel
						)
							.map(function(i, item) {
								return item.value;
						...
						var columnSequence = $(
							...
							.get();
						var lockColumnAmount =
							parseInt($('[name="lock"]:checked', this._settingPanel).val()) ||
							0;
					...
					_hideFilterPanel: function(evt) {
						...
						if (evt) {
							var target = $(evt.target);
							if (
								target.hasClass("yom-data-grid-filter-icon") ||
								target.closest(".yom-data-grid-filter-icon").length
							) {
								return;
							}
							if (
								(target.hasClass("yom-data-grid-filter-panel") ||
									target.closest(".yom-data-grid-filter-panel").length) &&
								target.attr("data-toggle") != "yom-data-grid-filter-panel"
							) {
								return;
							}
						}
					...
					_showFilterErrMsg: function(msg) {
						$(".alert-danger", this._filterPanel)
							.html(msg)
							.removeClass("hidden");
					},
					...
					_submitFilterForm: function() {
						var self = this;
						var findEmpty = !!$('[name="findEmpty"]', this._filterPanel).prop(
							"checked"
						);
							...
							if (filterOption.type == "set") {
								...
								if (filterOption.autoComplete) {
									...
									value.forEach(function(id) {
										valueMap[id] =
											(self._opt.getOptionNameById &&
												self._opt.getOptionNameById(id)) ||
											1;
									});
								} else {
									var set = $(".filter-option input", this._filterPanel)
								...
								if (compareType == "range") {
									...
									if (isNaN(fromValue) && isNaN(toValue)) {
										this._showFilterErrMsg(
											this._i18n.atLeastOneRangeValueRequired
										);
										return;
									}
								...
								filterOption.type == "datetime" ||
								filterOption.type == "month"
							) {
								var dateFromDom = $(".date-from", this._filterPanel);
								var dateToDom = $(".date-to", this._filterPanel);
								if (dateFromDom.attr("data-value")) {
									filterCriteria.fromValue = parseInt(
										dateFromDom.attr("data-value")
									);
									filterCriteria.fromDisplay = dateFromDom.find("input").val();
								}
								if (dateToDom.attr("data-value")) {
									filterCriteria.toValue = parseInt(
										dateToDom.attr("data-value")
									);
								...
								if (filterCriteria.fromValue > filterCriteria.toValue) {
									this._showFilterErrMsg(
										this._i18n.startDateCanNotLaterThanEndDate
									);
									return;
								}
							} else {
								...
								if (/;/.test(valueEl.val())) {
									this._showFilterErrMsg(
										this._i18n.filterValueContainIllegalChar
									);
									return;
								}
					...
					_updateColumnSortBtnStatus: function() {
						var selectedEl = $(
							".yom-data-grid-setting-column-item.selected",
							self._container
						);
						var hasPrev = selectedEl.prev().length;
						var hasNext = selectedEl.next().length;
						if (hasPrev) {
							$(".yom-data-grid-setting-btn-move-up", this._container)
						...
						if (hasNext) {
							$(".yom-data-grid-setting-btn-move-down", this._container)
								.removeClass("disabled")
								.prop("disabled", false);
						} else {
							$(".yom-data-grid-setting-btn-move-down", this._container)
					...
					_updateColumnSortScroll: function() {
						...
						if (top < selectedHeight) {
							container.prop(
								"scrollTop",
								container.prop("scrollTop") + Math.abs(top) - selectedHeight
							);
						}
					},
					...
					_bindEvent: function() {
						var self = this;
						this._container
							.delegate(".yom-data-grid-sortable", "click", function(evt) {
								var columnId = $(this)
									.closest("[data-column-id]")
									.attr("data-column-id");
								var sortOrder = $(".yom-data-grid-sort-arrow-down", this).length
									? "asc"
									: "desc";
							...
							.delegate(".yom-data-grid-btn-confirm-setting", "click", function(
								evt
							) {
								self._submitSettingForm();
							})
							.delegate(".yom-data-grid-setting-column-item", "click", function(
								evt
							) {
								$(
							...
							.delegate(
								".yom-data-grid-setting-column-item input",
								"click",
								function(evt) {
							...
							.delegate(
								".yom-data-grid-setting-column-all input",
								"click",
								function(evt) {
							...
							.delegate(".yom-data-grid-setting-btn-move-up", "click", function(
								evt
							) {
								var selectedEl = $(
									".yom-data-grid-setting-column-item.selected",
							...
							.delegate(".yom-data-grid-filter-remove-icon", "click", function(
								evt
							) {
								var cell = $(this).closest("[data-column-id]");
								var columnId = cell.attr("data-column-id");
							...
							.delegate(
								".yom-data-grid-check-box, .yom-data-grid-check-box-all",
								"click",
								function(evt) {
									...
									if (!(rowIndex >= 0)) {
										...
										if (checked) {
											if (
												self._opt.maxSelection > 0 &&
												self.getSelectedIndex().length > self._opt.maxSelection
											) {
												this.checked = false;
												self._opt.onExceedMaxSelection &&
													self._opt.onExceedMaxSelection(
														self._opt.maxSelection
													);
												return;
											}
											...
											$(
												...
												self._container
											).addClass("yom-data-grid-row-checked");
										} else {
											allChecked = false;
											...
											$(
												'[data-grid-row="' + rowIndex + '"]',
												self._container
											).removeClass("yom-data-grid-row-checked");
										}
									}
									if (self._opt.onSelect) {
										self._opt.onSelect(
											rowIndex,
											checked,
											(rowIndex >= 0 && self._data[rowIndex]) || undefined,
											allChecked
										);
									}
								}
							...
							.delegate(
								".yom-data-grid-row-clickable, .yom-data-grid-cell-clickable",
								"mousedown",
								function(evt) {
									self._lastRowMousedownPos = [evt.pageX, evt.pageY];
								}
							)
							.delegate(".yom-data-grid-cell-clickable", "click", function(
								evt
							) {
								var target = evt.target;
								...
								var columnId = $(target)
									.closest("[data-grid-column-id]")
									.attr("data-grid-column-id");
								var column = self.getColumnById(columnId);
								var rowIndex = +$(target)
								...
								if (rowIndex >= 0) {
									...
									isHeaderRow = true;
									rowIndex = +$(target)
										.closest("[data-grid-header-row]")
										.attr("data-grid-header-row");
							...
							.delegate(".yom-data-grid-row-clickable", "click", function(evt) {
								...
								self._lastRowClickTime = rowClickTime;
								if (
									rowClickTime - lastRowClickTime < 300 ||
									Math.abs(evt.pageX - self._lastRowMousedownPos[0]) > 3 ||
									Math.abs(evt.pageY - self._lastRowMousedownPos[1]) > 3
								) {
									return;
								}
								var clickable =
									$(target).closest(
										".yom-data-grid-sequence-cell, .yom-data-grid-checkbox-cell, .yom-data-grid-form-cell"
									).length === 0;
						...
						this._filterPanel
							...
							.delegate(".btn-check-all", "click", function(evt) {
								$(".set-container input", self._filterPanel).each(function(
									i,
									item
								) {
									item.checked = true;
						...
						if (this._opt.hightLightRow) {
							this._container
								.delegate("[data-grid-row]", "mouseenter", function(evt) {
									$("[data-grid-row]", self._container).removeClass(
										"yom-data-grid-row-hl"
									);
									$(
										'[data-grid-row="' + $(this).attr("data-grid-row") + '"]',
										self._container
									).addClass("yom-data-grid-row-hl");
								})
								.delegate("[data-grid-row]", "mouseleave", function(evt) {
									$(
										'[data-grid-row="' + $(this).attr("data-grid-row") + '"]',
										self._container
									).removeClass("yom-data-grid-row-hl");
								})
								.delegate("[data-grid-row]", "click", function(evt) {
									$("[data-grid-row]", self._container).removeClass(
										"yom-data-grid-row-click-hl"
									);
									$(
										'[data-grid-row="' + $(this).attr("data-grid-row") + '"]',
										self._container
									).addClass("yom-data-grid-row-click-hl");
								});
							this._container
								.delegate("[data-grid-header-row]", "mouseenter", function(
									evt
								) {
									$("[data-grid-header-row]", self._container).removeClass(
										"yom-data-grid-row-hl"
									);
									$(
										'[data-grid-header-row="' +
											$(this).attr("data-grid-header-row") +
											'"]',
										self._container
									).addClass("yom-data-grid-row-hl");
								})
								.delegate("[data-grid-header-row]", "mouseleave", function(
									evt
								) {
									$(
										'[data-grid-header-row="' +
					...
					_render: function(data, headerData, state, setting, opt) {
						...
						$(
							".yom-data-grid-header, .yom-data-grid-body",
							this._container
						).off("wheel", this._bind.scrollLocked);
						if (this._lockedBody) {
						...
						var width;
						if (
							this._allColumns.length - this._hiddenColumns.length <
								this._opt.minScrollXColumns ||
							this._width == "100%"
						) {
							noScrollX = true;
							width = "100%";
							...
							width =
								this._width == "auto" ? this._holder.width() : this._width;
						}
						...
						this._container.html(
							mainTpl.render({
								...
								width: width,
								noScrollX: noScrollX,
								lockedColumns: this._defaultLockedColumns.concat(
									this._lockedColumns
								),
						...
						this._lockedBody = $(
							".yom-data-grid-locked-columns .yom-data-grid-body",
							this._container
						)[0];
						this._scrollHeader = $(
							".yom-data-grid-columns .yom-data-grid-header",
							this._container
						)[0];
						this._scrollBody = $(
							".yom-data-grid-columns .yom-data-grid-body",
							this._container
						);
						this._scrollBody.on("scroll", this._bind.scroll);
						$(".yom-data-grid-header, .yom-data-grid-body", this._container).on(
							"wheel",
							this._bind.scrollLocked
						);
						this._settingPanel = $(
						...
						this._firstRender();
						if (
							this._tmp.reactElements.length ||
							this._tmp.headerReactElements.length
						) {
							window.require(["react-dom"], function(ReactDOM) {
								$("[data-react-element]", that._container).each(function(
									i,
									el
								) {
									...
									var reactElements =
										attrs[2] == "header"
											? that._tmp.headerReactElements
											: that._tmp.reactElements;
					...
					isAllChecked: function() {
						...
						$(".yom-data-grid-check-box[data-row-index]", this._container).each(
							function(i, item) {
					...
					showSettingPanel: function() {
						this._settingPanel.html(
							settingPanelTpl.render({
					...
					showFilterPanel: function(column, target, align) {
						...
						window.require(["yom-auto-complete"], function(YomAutoComplete) {
							...
							self._filterPanel.html(
								filterPanelTpl.render(
									{
										i18n: self._i18n,
										column: column,
										...
										dateWithoutYear: self._opt.dateWithoutYear
									},
									{
							...
							if (type == "set" && filterOption.autoComplete) {
								var filterCriteria =
									self._filterMap[self.getFilterKeyById(column.id)] || {};
								var valueMap = filterCriteria.valueMap || {};
								var box = $(".auto-complete-box", self._filterPanel);
								self._filterPanel.show();
								var autoComplete = new YomAutoComplete(
									box,
									$.extend(
										{
											...
											initData: Object.keys(valueMap).map(function(id) {
												return {
													id: id,
													name:
														typeof valueMap[id] == "string" ? valueMap[id] : id
												};
											}),
								...
								self._filterPanel
									.find('[name="compareType"]')
									.on("change", function(evt) {
										var formAndToValueEl = $(
											'[name="fromValue"], [name="toValue"]',
											self._filterPanel
										).closest(".form-group");
										var valueEl = $(
											'[name="value"]',
								...
								$('[name="value"]', self._filterPanel).on("paste", function(
									evt
								) {
									var text = "";
									try {
							...
							if (
								(align == "right" &&
									left - containerOffset.left > filterPanelWidth) ||
								left + filterPanelWidth > containerOffset.left + containerWidth
							) {
					...
					setColumns: function(columns, setting) {
						setting = setting || {};
						this._lockColumnAmount = Math.min(
							this._MAX_LOCKED_COLUMNS,
							setting.lockColumnAmount >= 0
								? setting.lockColumnAmount
								: this._lockColumnAmount
						);
						this._columnSequence =
							(Array.isArray(setting.columnSequence) &&
								setting.columnSequence.concat()) ||
							this._columnSequence;
						this._hiddenColumns =
							(Array.isArray(setting.hiddenColumns) &&
								setting.hiddenColumns.concat()) ||
							this._hiddenColumns;
						var self = this;
							...
							if (column) {
								...
								if (column.width > 0) {
									column.width = Math.min(
										Math.max(column.width, self._MIN_COLUMN_WIDTH),
										self._MAX_COLUMN_WIDTH
									);
								}
								if (
									column.searchOnly ||
									self._hiddenColumns.indexOf(column.id) >= 0
								) {
									column.hidden = true;
								} else {
									column.hidden = false;
					...
					getColumnByFilterKey: function(key) {
						var column = this._allColumns.filter(function(item) {
							return (
								item.filterable && (item.filterKey == key || item.id == key)
							);
						})[0];
						return column;
					},
					...
					getSelectedIndex: function() {
						var self = this;
						var res = [];
						$(".yom-data-grid-check-box", this._container).each(function(
							i,
							item
						) {
					...
					getSelection: function(rowIndex) {
						var checkbox = $(
							'[data-grid-row="' + rowIndex + '"] .yom-data-grid-check-box'
						)[0];
					...
					setSelection: function(rowIndex, checked) {
						...
						if (
							checked &&
							this._opt.maxSelection > 0 &&
							this.getSelectedIndex().length >= this._opt.maxSelection
						) {
							this._opt.onExceedMaxSelection &&
								this._opt.onExceedMaxSelection(this._opt.maxSelection);
							return false;
						}
						var checkbox = $(
							'[data-grid-row="' + rowIndex + '"] .yom-data-grid-check-box'
						...
						if (checked) {
							...
							$('[data-grid-row="' + rowIndex + '"]', this._container).addClass(
								"yom-data-grid-row-checked"
							);
						} else {
							allChecked = false;
					...
					hightLightRow: function(index, className) {
						$('[yom-data-grid-row="' + index + '"]', this._container).addClass(
							className || "yom-data-grid-row-error"
						);
					},

					dehightLightRows: function(className) {
						$("[yom-data-grid-row]", this._container).removeClass(
							className || "yom-data-grid-row-error"
						);
					},
								...
								.forEach(function(item) {
									...
									if (column) {
										...
										if (filterCriteria.findEmpty) {
											...
											if (filterOption.type == "set") {
												value = parts;
												var options = self.normalizeFilterOptions(
												...
												if (compareType == "range") {
													...
													filterCriteria.fromValue = fromValue;
													filterCriteria.toValue = toValue;
													filterCriteria.displayValue =
														(self._i18n[compareType] || "") +
														" " +
														(fromValue === 0 ? 0 : fromValue || "") +
														" ~ " +
														(toValue === 0 ? 0 : toValue || "");
												} else {
												...
												var fromValue = (filterOption.parser || parseInt)(
													parts.shift()
												);
												var toValue = (filterOption.parser || parseInt)(
													parts.shift()
												);
												if (fromValue) {
													filterCriteria.fromValue = fromValue;
													filterCriteria.fromDisplay = filterOption.formatter(
														fromValue
													);
												}
												if (toValue) {
													filterCriteria.toValue = toValue;
													filterCriteria.toDisplay = filterOption.formatter(
														toValue
													);
												}
												filterCriteria.displayValue =
													(fromValue ? filterCriteria.fromDisplay : "") +
													" ~ " +
													(toValue ? filterCriteria.toDisplay : "");
											} else {
												value = parts.join(",");
												filterCriteria.value = value;
					...
					export: function(data, fileName, format) {
						...
						var columns = this._lockedColumns
							.concat(this._scrollColumns)
					...
					render: function(data, headerData, state, setting, opt) {
						...
						this._unmoundReactComponents(
							this._render.bind(this, data, headerData, state, setting, opt)
						);
					},
